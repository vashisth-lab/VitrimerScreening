...
def get_labeled_rdkit(file):
    ...
    desc = [Descriptors.CalcMolDescriptors(m) for m in mols]
    df_desc = pd.DataFrame(desc)

    d = {'acid': acid, 'epoxide': epoxide, 'tg': tg}
    for col in df_desc:
        d[col] = df_desc[col].to_list()
    pd.DataFrame(d).to_csv(f'data/{file}_rdkit.csv', index=False, float_format='%.4f')


def get_unlabeled_rdkit(file, i_batch):
    ...
    for col in df_desc:
        d[col] = df_desc[col].to_list()
    pd.DataFrame(d).to_csv(f'data/{file}_rdkit/{file}_rdkit_{i_batch:d}.csv', index=False, float_format='%.4f')


# preprocess labeled dataset
get_labeled_rdkit('labeled')

# preprocess hypothetical unlabeled dataset
file = 'unlabeled'
os.mkdir(f'data/{file}_rdkit')

for i in range(100):
    get_unlabeled_rdkit(file, i)

desc = []
df = pd.read_csv('data/labeled_rdkit.csv')
...
for i in range(100):
    df = pd.read_csv(f'data/{file}_rdkit/{file}_rdkit_{i:d}.csv')
    for col in desc:
...
df1.to_csv('data/labeled_rdkit.csv', index=False, float_format='%.4f')
np.random.seed(0)
idx = np.random.permutation(len(df1))
train_idx = idx[:int(0.9*len(df1))]
test_idx = idx[int(0.9*len(df1)):]
df1.iloc[train_idx, :].to_csv('data/labeled_rdkit_train.csv', index=False, float_format='%.4f')
df1.iloc[test_idx, :].to_csv('data/labeled_rdkit_test.csv', index=False, float_format='%.4f')
...
df = pd.read_csv(f'data/{file}_rdkit/{file}_rdkit_0.csv')
for col in desc:
    if not pd.to_numeric(df[col], errors='coerce').notnull().all():
        desc.remove(col)
